...
DocPanel = Ext.extend(Ext.Panel, {
    ...
    initComponent : function(){
        var ps = this.cclass.replace(/^cls-/,'').split('.');
	...
	scrollToSection : function(id){
		...
		if(el){
			...
			this.body.scrollTo('top', top-25, {duration:.75, callback: function(member){
                if (Ext.fly(el).next('h2')) {
                	Ext.fly(el).next('h2').pause(.2).highlight('#8DB2E3', {attr:'color'});
                } else {
                	var tr = Ext.get(el).up('tr');
...
Ext.extend(MainPanel, Ext.TabPanel, {
    ...
    loadClass : function(href, cls, member, iconCls){
        var id = 'docs-' + cls;
...
Ext.onReady(function(){
    ...
    api.on('click', function(node, e){
         if(node.isLeaf()){
            e.stopEvent();
            mainPanel.loadClass(node.attributes.href, node.id, '', node.attributes.iconCls);
         }
    });
    ...
    if(page){
        ...
        var cls = ps['class'];
        mainPanel.loadClass('symbols/' + cls + '.html', cls, ps.member);
    }
