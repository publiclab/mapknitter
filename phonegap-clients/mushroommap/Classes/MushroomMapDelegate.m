#import "MushroomMapDelegate.h"
#import "MushroomMapViewController.h"
#import <UIKit/UIKit.h>

@implementation MushroomMapDelegate
...
/**
 Returns an instance of a MushroomMapCommand object, based on its name.  If one exists already, it is returned.
 */
...
{	
	 ...
	 * MushroomMap.plist
	 *
	 * This block of code navigates to the MushroomMap.plist in the Config Group and reads the XML into an Hash (Dictionary)
	 *
	 */
    NSDictionary *temp = [MushroomMapDelegate getBundlePlist:@"MushroomMap"];
	 ...
	 */
	// device testing:
    NSURL *appURL        = [NSURL URLWithString:@"http://dhcp-23-117.media.mit.edu:3000/mushrooms/"];
    // local testing:
     ...
     * only use landscape or portrait orientations, change the value in MushroomMap.plist to indicate that.
...
- (void)webViewDidStartLoad:(UIWebView *)theWebView {
	 ...
	 */
    NSDictionary *temp = [MushroomMapDelegate getBundlePlist:@"Settings"];
...
- (BOOL)webView:(UIWebView *)theWebView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
     ...
     */
     if ([[url scheme] isEqualToString:@"gap"]) {
        ...
        [theWebView stringByEvaluatingJavaScriptFromString:@"MushroomMap.queue.ready = true;"];
        ...
        if (components.count == 2)
        {
            ...
            MushroomMapCommand* obj = [self getCommandInstance:className];
    ...
    /*
     * We don't have a MushroomMap or local file request, load it in the main Safari browser.
